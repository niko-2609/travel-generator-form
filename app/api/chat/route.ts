...
export async function POST(req: Request) {
  ...
  if (validatedData?.error) {
    return NextResponse.json({ error: "Please enter valid data" })
  }
  ...
  if (!travel_dates) NextResponse.json({ error: "Please enter valid data" })
  // this will pass a reference of the array created in this function
  ...
  // for (const key in promptBodyParameters) {
  //     for (const word of forbiddenWords) {
  //       if (promptBodyParameters[key].includes(word)) {
  //         return {error: `Malicious activitity detected.`};
  //       }
  //     }
  //   }
  ...
  const chatCompletion = await openai.chat.completions.create({
    model: model, // Define the model here
    stream: true,
    temperature: 0,
...
//   const validatedData = PromptSchema.safeParse(values)
//   if (validatedData?.error) {
